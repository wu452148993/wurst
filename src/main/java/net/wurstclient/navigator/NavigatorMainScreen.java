...
public final class NavigatorMainScreen extends NavigatorScreen
{
	...
	protected void onResize()
	{
		...
		searchBar =
			new TextFieldWidget(tr, 0, 32, 200, 20, new StringTextComponent(""));
	...
	protected void onMouseClick(double x, double y, int button)
	{
		...
		{
			WurstClient.MC.displayGuiScreen((Screen)null);
			return;
		}
							...
							float partialTicks)
	{
		...
		if(!clickTimerRunning)
		{
			...
			WurstClient.MC.fontRenderer.drawString(matrixStack, "Search: ",
		...
		if(tooltip != null)
		{
			...
			for(int i = 0; i < lines.length; i++)
				fr.drawString(matrixStack, lines[i], xt1 + 2,
					yt1 + 1 + i * fr.FONT_HEIGHT, 0xffffff);
		}
	}
		...
		float partialTicks, int i, int x, int y)
	{
		...
		if(!clickTimerRunning)
		{
			...
			minecraft.fontRenderer.drawString(matrixStack, buttonText, area.x + 4,
				area.y + 4, 0xffffff);
			GL11.glDisable(GL11.GL_TEXTURE_2D);
			GL11.glEnable(GL11.GL_BLEND);
		}
