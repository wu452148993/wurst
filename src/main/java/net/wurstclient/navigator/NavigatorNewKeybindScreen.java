...
public class NavigatorNewKeybindScreen extends NavigatorScreen
{
	...
	protected void onResize()
	{
		...
		okButton = new Button(width / 2 - 151, height - 65, 149, 18,
			new StringTextComponent("OK"), b -> {
				if(choosingKey)
				{
					...
					WurstClient.MC.displayGuiScreen(parent);
				}else
				{
					choosingKey = true;
		...
		// cancel button
		addButton(new Button(width / 2 + 2, height - 65, 149, 18,
			new StringTextComponent("Cancel"), b -> WurstClient.MC.displayGuiScreen(parent)));
	}
	...
	protected void onKeyPress(int keyCode, int scanCode, int int_3)
	{
		...
		{
			selectedKey =
					InputMappings.getInputByCode(keyCode, scanCode).getTranslationKey();
							...
							float partialTicks)
	{
		...
		drawCenteredString(matrixStack, minecraft.fontRenderer, "New Keybind",
			middleX, 32, 0xffffff);
		...
		if(!choosingKey)
		{
			...
			for(PossibleKeybind pkb : possibleKeybinds)
			{
				...
				drawString(matrixStack, minecraft.fontRenderer,
					pkb.getCommand(), x1 + 1,
					y1 + 1 + minecraft.fontRenderer.FONT_HEIGHT, 0xffffff);
				GL11.glDisable(GL11.GL_TEXTURE_2D);
				GL11.glEnable(GL11.GL_BLEND);
			}
		}
