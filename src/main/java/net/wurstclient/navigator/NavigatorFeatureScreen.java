...
public final class NavigatorFeatureScreen extends NavigatorScreen
{
	...
	protected void onResize()
	{
		...
		if(hasPrimaryAction)
		{
			primaryButton = new Button(width / 2 - 151, height - 65,
				hasHelp ? 149 : 302, 18, new StringTextComponent(primaryAction), b -> {
					...
					primaryButton.setMessage(
						new StringTextComponent(feature.getPrimaryAction()));
		...
		if(!possibleKeybinds.isEmpty())
		{
			...
			ButtonData addKeybindButton =
					...
					area.y + getStringHeight(text) - 7, 12, 8, "+", 0x00ff00)
				{
					...
					{
						// add keybind
						WurstClient.MC.displayGuiScreen(new NavigatorNewKeybindScreen(
			...
			else
			{
					...
					addKeybindButton.height, "-", 0xff0000)
				{
					...
					{
						// remove keybind
						minecraft.displayGuiScreen(new NavigatorRemoveKeybindScreen(
	...
	protected void onMouseClick(double x, double y, int button)
	{
		...
		{
			minecraft.getSoundHandler().play(
					SimpleSound.master(SoundEvents.UI_BUTTON_CLICK, 1));
			activeButton.press();
			WurstClient.INSTANCE.getNavigator()
				.addPreference(feature.getName());
			return;
							...
							float partialTicks)
	{
		...
		drawCenteredString(matrixStack, minecraft.fontRenderer, feature.getName(),
			middleX, 32, 0xffffff);
		...
		{
			drawString(matrixStack, minecraft.fontRenderer, line,
				bgx1 + 2, textY, 0xffffff);
			textY += minecraft.fontRenderer.FONT_HEIGHT;
		}
		GL11.glEnable(GL11.GL_BLEND);
