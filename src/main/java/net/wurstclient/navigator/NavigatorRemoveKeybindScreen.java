...
public class NavigatorRemoveKeybindScreen extends NavigatorScreen
{
	...
	{
		// OK button
		removeButton = new Button(width / 2 - 151, height - 65, 149, 18,
			new StringTextComponent("Remove"), b -> remove());
		removeButton.active = !selectedKey.isEmpty();
		addButton(removeButton);
		
		// cancel button
		addButton(new Button(width / 2 + 2, height - 65, 149, 18,
			new StringTextComponent("Cancel"), b -> minecraft.displayGuiScreen(parent)));
	}
							...
							float partialTicks)
	{
		...
		drawCenteredString(matrixStack, minecraft.fontRenderer, "Remove Keybind",
			middleX, 32, 0xffffff);
