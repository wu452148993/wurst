...
public final class TabGui implements KeyPressListener
{
	...
	private void updateSize()
	{
		...
		for(Tab tab : tabs)
		{
			int tabWidth = WurstClient.MC.fontRenderer.getStringWidth(tab.name) + 10;
			if(tabWidth > width)
				width = tabWidth;
		}
		height = tabs.size() * 10;
	...
	public void render(MatrixStack matrixStack, float partialTicks)
	{
		...
		double factor = sr.getGuiScaleFactor();
		...
		for(int i = 0; i < tabs.size(); i++)
		{
			...
			WurstClient.MC.fontRenderer.drawString(matrixStack, tabName, 2, textY,
				0xffffffff);
			textY += 10;
		}
		GL11.glEnable(GL11.GL_BLEND);
		...
		if(tabOpened)
		{
			...
			for(int i = 0; i < tab.features.size(); i++)
			{
				...
				WurstClient.MC.fontRenderer.drawString(matrixStack, fName, 2,
					tabTextY, 0xffffffff);
				tabTextY += 10;
			}
			GL11.glEnable(GL11.GL_BLEND);
	...
	private static final class Tab
	{
		...
		public void updateSize()
		{
			...
			{
				int fWidth =
					WurstClient.MC.fontRenderer.getStringWidth(feature.getName())
						+ 10;
				if(fWidth > width)
					width = fWidth;
			}
