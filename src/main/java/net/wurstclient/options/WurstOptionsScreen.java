...
public class WurstOptionsScreen extends Screen
{
	...
	private void addManagerButtons()
	{
		...
		new WurstOptionsButton(-50, 24, () -> "Keybinds",
			...
			b -> minecraft.displayGuiScreen(new KeybindManagerScreen(this)));
		...
		new WurstOptionsButton(-50, 72, () -> "Zoom",
			...
			b -> minecraft.displayGuiScreen(new ZoomManagerScreen(this)));
	}
	...
	private void addLinkButtons()
	{
		...
		new WurstOptionsButton(54, 24, () -> "Official Website",
			"WurstClient.net", b -> os.openURI("https://www.wurstclient.net/"));
		
		new WurstOptionsButton(54, 48, () -> "Wurst Wiki",
			"Wiki.WurstClient.net",
			b -> os.openURI("https://wiki.wurstclient.net/"));
		
		new WurstOptionsButton(54, 72, () -> "Twitter", "@Wurst_Imperium",
			b -> os.openURI("https://twitter.com/Wurst_Imperium"));
		
		new WurstOptionsButton(54, 96, () -> "Reddit", "r/WurstClient",
			b -> os.openURI("https://www.reddit.com/r/WurstClient/"));
		
		new WurstOptionsButton(54, 120, () -> "Donate",
			"paypal.me/WurstImperium",
			b -> os.openURI("https://www.wurstclient.net/donate/"));
	}
									 ...
									 int mouseY)
	{
		for(Widget button : buttons)
		{
			...
			func_243308_b(matrixStack, woButton.tooltip, mouseX, mouseY);
			break;
		}
	}
	...
	private final class WurstOptionsButton extends Button
	{
								  ...
								  IPressable pressAction)
		{
			super(WurstOptionsScreen.this.width / 2 + xOffset,
				...
				new StringTextComponent(messageSupplier.get()), pressAction);
			
			this.messageSupplier = messageSupplier;
			
			if(tooltip.isEmpty())
				this.tooltip = Arrays.asList(new StringTextComponent[0]);
			else
			{
				String[] lines = tooltip.split("\n");

				StringTextComponent[] lines2 = new StringTextComponent[lines.length];
				for(int i = 0; i < lines.length; i++)
					lines2[i] = new StringTextComponent(lines[i]);
		...
		{
			super.onPress();
			setMessage(new StringTextComponent(messageSupplier.get()));
		}
	}
}
