...
public final class EventManager
{
	...
	private <L extends Listener, E extends Event<L>> void fireImpl(E event)
	{
		...
		}catch(Throwable e)
		{
			...
			CrashReportCategory section = report.makeCategory("Affected event");
			section.addDetail("Event class", () -> event.getClass().getName());
	...
	public <L extends Listener> void add(Class<L> type, L listener)
	{
		...
		}catch(Throwable e)
		{
			...
			CrashReportCategory section = report.makeCategory("Affected listener");
			section.addDetail("Listener type", () -> type.getName());
			section.addDetail("Listener class", () -> listener.getClass().getName());
