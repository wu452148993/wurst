...
public final class RadarComponent extends Component
{
					   ...
					   float partialTicks)
	{
		...
		{
			double diffX =
				e.lastTickPosX + (e.getPosX() - e.lastTickPosX) * partialTicks
					- (player.lastTickPosX
						+ (player.getPosX() - player.lastTickPosX) * partialTicks);
			double diffZ =
				e.lastTickPosZ + (e.getPosZ() - e.lastTickPosZ) * partialTicks
					- (player.lastTickPosZ
			...
			double angle;
			if(hack.isRotateEnabled())
				angle = Math.toRadians(player.rotationYaw - neededRotation - 90);
			else
