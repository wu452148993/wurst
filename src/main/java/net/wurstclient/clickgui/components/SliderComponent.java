...
public final class SliderComponent extends Component
{
	...
	private void handleLeftClick()
	{
		if(Screen.hasControlDown())
			MC.displayGuiScreen(new EditSliderScreen(MC.currentScreen, setting));
		else
			dragging = true;
	}
		...
		int y1, boolean renderAsDisabled)
	{
		...
		FontRenderer tr = MC.fontRenderer;
		...
		tr.drawString(matrixStack, name, x1, y1 + 2, color);
		tr.drawString(matrixStack, value, x2 - valueWidth, y1 + 2, color);
