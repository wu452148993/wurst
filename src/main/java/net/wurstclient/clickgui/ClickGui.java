...
public final class ClickGui
{
	...
	public void init()
	{
		...
		int x = 5;
		int y = 5;
		net.minecraft.client.MainWindow sr = MC.getMainWindow();
		for(Window window : windows)
		{
			window.pack();
		...
		if(!tooltip.isEmpty())
		{
			...
			for(int i = 0; i < lines.length; i++)
				fr.drawString(matrixStack, lines[i], xt1 + 2,
					yt1 + 2 + i * fr.FONT_HEIGHT, 0xffffff);
			GL11.glEnable(GL11.GL_BLEND);
		...
		int mouseX, int mouseY, float partialTicks)
	{
		...
		if(!window.isMinimized())
		{
			...
			net.minecraft.client.MainWindow sr = MC.getMainWindow();
			int sf = (int)sr.getGuiScaleFactor();
		...
		FontRenderer fr = MC.fontRenderer;
		String title =
			fr.func_238417_a_(new StringTextComponent(window.getTitle()), x3 - x1)
				.getString();
		fr.drawString(matrixStack, title, x1 + 2, y1 + 3, 0xf0f0f0);
		GL11.glEnable(GL11.GL_BLEND);
	}
