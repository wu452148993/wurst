...
public final class AddBookOfferScreen extends Screen
{
	...
	public void init()
	{
		...
		addDrawableChild(levelPlusButton = new ButtonWidget(width / 2 + 2,
			height - 74, 20, 12, Text.literal("+"), b -> updateLevel(1, true)));
		levelPlusButton.active = false;
		
		addDrawableChild(
			levelMinusButton = new ButtonWidget(width / 2 + 26, height - 74, 20,
				12, Text.literal("-"), b -> updateLevel(-1, true)));
		levelMinusButton.active = false;
		
		addDrawableChild(pricePlusButton = new ButtonWidget(width / 2 + 2,
			height - 58, 20, 12, Text.literal("+"), b -> updatePrice(1, true)));
		pricePlusButton.active = false;
		
		addDrawableChild(
			priceMinusButton = new ButtonWidget(width / 2 + 26, height - 58, 20,
				12, Text.literal("-"), b -> updatePrice(-1, true)));
		priceMinusButton.active = false;
		
		addDrawableChild(addButton = new ButtonWidget(width / 2 - 102,
			height - 28, 100, 20, Text.literal("Add"), b -> {
		...
		addDrawableChild(
			cancelButton = new ButtonWidget(width / 2 + 2, height - 28, 100, 20,
	...
	private static class ListGui extends ListWidget
	{
		...
		public ListGui(MinecraftClient mc, AddBookOfferScreen screen)
		{
			...
			this.mc = mc;
			this.screen = screen;
			list = Registry.ENCHANTMENT.stream().map(BookOffer::create)
