...
public final class EditBookOfferScreen extends Screen
{
	...
	public void init()
	{
		...
		addDrawableChild(levelPlusButton = new ButtonWidget(width / 2 + 2, 110,
			20, 12, Text.literal("+"), b -> updateLevel(1, true)));
		levelPlusButton.active = false;
		
		addDrawableChild(levelMinusButton = new ButtonWidget(width / 2 + 26,
			110, 20, 12, Text.literal("-"), b -> updateLevel(-1, true)));
		levelMinusButton.active = false;
		
		addDrawableChild(pricePlusButton = new ButtonWidget(width / 2 + 2, 126,
			20, 12, Text.literal("+"), b -> updatePrice(1, true)));
		pricePlusButton.active = false;
		
		addDrawableChild(priceMinusButton = new ButtonWidget(width / 2 + 26,
			126, 20, 12, Text.literal("-"), b -> updatePrice(-1, true)));
		priceMinusButton.active = false;
		
		addDrawableChild(saveButton = new ButtonWidget(width / 2 - 102,
			height / 3 * 2, 100, 20, Text.literal("Save"), b -> {
		...
		addDrawableChild(
			cancelButton = new ButtonWidget(width / 2 + 2, height / 3 * 2, 100,
		...
		float partialTicks)
	{
		...
		Item item = Registry.ITEM.get(new Identifier("enchanted_book"));
